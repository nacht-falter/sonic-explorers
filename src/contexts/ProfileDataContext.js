...
import { unfollowHelper, followHelper } from "../utils/utils";
...
export const ProfileDataProvider = ({ children }) => {
  ...
  const handleFollow = async (clickedProfile) => {
    try {
      const { data } = await axiosResponse.post("/follows/", {
  ...
  const handleUnfollow = async (clickedProfile) => {
    try {
      await axiosResponse.delete(`/follows/${clickedProfile.follow_id}`);
      setProfileData((prevState) => ({
        ...prevState,
        pageProfile: {
  ...
  useEffect(() => {
    const handleMount = async () => {
      try {
        const { data } = await axiosRequest.get("/profiles/?ordering=-followers_count");
        setProfileData((prevState) => ({
          ...prevState,
